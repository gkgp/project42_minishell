...
/*   Updated: 2022/03/09 14:55:17 by min-kang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	cmd_execute(t_node *node, int fd_in, int fd_out, char **envp)
{
	t_redir	redir;
	char	*cmd_path;
...
static int	execute_loop(t_node *node, char **envp, int fd_in)
{
	int		fd[2];
	pid_t	pid;
